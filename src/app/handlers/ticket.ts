...
import { ActionRowBuilder, ButtonBuilder, ButtonInteraction, Collection, EmbedBuilder, ModalSubmitInteraction, StringSelectMenuInteraction, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, MessageCreateOptions, Guild, GuildBasedChannel, Interaction, SendableChannels, TextBasedChannel, DMChannel, GuildTextBasedChannel, FetchMessagesOptions, User, MessageFlags, ChannelType, PermissionFlagsBits, CategoryChannel, ButtonStyle, MessageActionRowComponentBuilder } from "discord.js";
...
export default class TicketHandler extends Handler {
    ...
    public async load() {
        ...
        channel.send(this.createMenuPayload());
    }
    ...
    public createMenuPayload() {
        ...
        const selectOptions = this.categories.map((ticketType, ticketName) => {
    ...
    private loadMenu(): TicketMenu {
        ...
        return YAML.parseDocument(menu, { merge: true }).toJS();
    }
    ...
    private loadCategories(): Collection<string, TicketType> {
        ...
        for (const ticketFilePath of ticketFiles) {
            ...
            const ticket: TicketType = YAML.parseDocument(ticketFile, { merge: true }).toJS();
            ...
            if (data) {
                ...
                if (ticketOwner) {
                    ...
                    for (let i = 0; i < logEmbeds.length; i++) {
                        ...
                        ticketOwner.send({ embeds: logEmbeds[i] })
                    }
                }
                for (let i = 0; i < logEmbeds.length; i++) {
                    await new Promise((resolve) => setTimeout(resolve, 500));
                    this.logChannel.send({ embeds: logEmbeds[i] })
                }
            }
